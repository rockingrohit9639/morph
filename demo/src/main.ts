...
function renderSidebar() {
  ...
  const sorted = [...SIDEBAR_ITEMS].sort((a, b) => {
  ...
  container.innerHTML = sorted
    .map((item) => {
      ...
      const textColor = isActive ? "text-neutral-900 font-medium" : data ? "text-neutral-600" : "text-neutral-400";
...
function renderCommands(filter = "") {
  ...
  list.innerHTML = filtered
    .map((cmd) => {
      ...
      return `
        <button class="flex items-center justify-between w-full px-4 py-2 text-sm border-b border-neutral-100 last:border-0 hover:bg-neutral-50 text-left transition-colors ${isActive ? "text-neutral-900" : "text-neutral-600"}" data-id="${cmd.id}">
          <span>${cmd.label}</span>
...
function renderToolbar() {
  ...
  const sorted = [...TOOLBAR_ACTIONS].sort((a, b) => {
    const rankA = ranked.find((r) => r.id === a.id)?.rank ?? Infinity;
    const rankB = ranked.find((r) => r.id === b.id)?.rank ?? Infinity;
    return rankA - rankB;
  });
  ...
  container.innerHTML = sorted
    .map((action) => {
      ...
      return `
        <button class="w-9 h-9 flex items-center justify-center text-xs font-mono border ${style} transition-all" data-id="${action.id}">
          ${action.label}
        </button>
      `;
    })
